...
class data_base:
  ...
  def add_lesson(self, cabinet: str, group: int, teacher: str, time_start: str, time_end: str, name: str, date: str, regularity: bool) -> bool:
    try:
      if (not self.is_lesson_in_table(cabinet, time_start, date) and cabinet in self.validation_cabinet):
        self.cursor.execute('INSERT INTO Class VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (cabinet, group, teacher, time_start, time_end, name, date, regularity, 0))
        self.connection.commit()
        return True
    except sqlite3.Error as error:
  ...
  def delete_lesson(self, cabinet, time_start, date) -> bool:
    try:
      self.cursor.execute('DELETE FROM Class WHERE Cabinet = ? AND TimeStart = ? AND Date = ?', (cabinet, time_start, date))
      self.connection.commit()
    except sqlite3.Error as error:
  ...
  def is_lesson_in_table(self, cabinet, time_start, date) -> bool:
    try:
      return (len(self.cursor.execute("SELECT Cabinet, TimeStart, Date FROM Class WHERE Cabinet = ? AND TimeStart = ? AND Date = ?", (cabinet, time_start, date)).fetchall()) > 0)
    except sqlite3.Error as error:
      print("Ошибка выбора")
      return None
  ...
  def get_lesson(self, cabinet, time_start, date):
    try:
      row = self.cursor.execute("SELECT * FROM Class WHERE Cabinet = ? AND TimeStart = ? AND Date = ?", (cabinet, time_start, date)).fetchall()
      if len(row) > 0:
